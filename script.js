...
function setupStaticListeners() {
  ...
  document.addEventListener('click', event => {
    const attributeCard = event.target.closest('.attribute-card');
    const interactiveInsideAttribute = event.target.closest('input, textarea, button, select, label');
    if (attributeCard && !interactiveInsideAttribute) {
      const attributeId = attributeCard.dataset.id;
      if (attributeId) {
